...
from SemanticSegmentation.callSS import execSS
...
for now in range(0, 2949):
    ...
    time_semseg = time.time()
    # Add to pipeline
    label = np.array(execSS(img))
    print("--- SS %s seconds ---" % (time.time() - time_semseg))
    ...
    for idx in range(len(lst)):
        ...
        try:
            ...
            if len(choose) > num_points:
                ...
                #choose = np.pad(choose, (0, num_points - len(choose)), 'wrap') # 'constant' or 'empty'
                choose = np.pad(choose, (0, num_points - len(choose)), 'constant') # 'constant' or 'empty'
            ...
            my_mask = torch.from_numpy(my_mask.astype(np.float32))
            my_mask = Variable(my_mask).cuda()
            my_mask = my_mask.view(1, 3, my_mask.size()[1], my_mask.size()[2])
            ...
            time_pose = time.time()
            pred_r, pred_t, pred_c, emb = estimator(img_masked, cloud, choose, index, my_mask)
            print("--- PE %s seconds ---" % (time.time() - time_pose))
